...
- **Reads are open, but capture location is not public.** Listing and viewing images are not auth-gated — the library is shared. Capture GPS is more sensitive, so on a **secured** server the per-image `lat`/`lon` are stripped from the listing and the raw EXIF endpoint (`GET /images/:id/exif`) requires a logged-in user (`canReadSensitiveMetadata` in `sk-request.ts`); an **unsecured** server leaves everything open. If you add a route that exposes per-image metadata, gate location the same way; if it mutates state, gate it for write access.
- **The v2 resource provider has no request principal, so it withholds location unconditionally.** `registerResourceProvider`'s methods (`src/images/image-resources.ts`) are called by the server without any `req`, so there is no logged-in user to authorize a location read against. The resource docs therefore **never** include `lat`/`lon`, for anyone. The provider is also read-only — `setResource`/`deleteResource` reject — so uploads and deletes stay on the REST route where content validation and the write gate live. Do not add sensitive fields (raw EXIF, GPS) to a resource doc; there is no way to gate them there.
