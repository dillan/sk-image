...
- **Read paths are open by design.** Listing and viewing images (including per-image EXIF, which can carry capture GPS coordinates) are not auth-gated — the library is meant to be shared across the boat, so any client that can read Signal K data can read them. Only the state-changing routes (upload, delete, purge) require write permission. If you add a route that mutates state, it gates itself the same way; if you add one that only reads, keep it consistent with the existing open read paths.
