...
> 5. **Upload, delete, and purge require an authenticated principal** when the server has security enabled.
...
| Unauthenticated mutation | upload / delete / cache-purge gated on an authenticated principal when server security is on | `src/images/image-router.ts` |
...
- **The auth gate is defensive, on top of the server's own middleware.** Signal K's security strategy already fronts these routes; the in-handler principal check in `src/images/image-router.ts` is a second, explicit gate so a misconfigured or reordered middleware chain can't quietly open upload, delete, or purge. It feature-detects the security strategy and fails closed.
- **Read paths are open by design.** Listing and viewing images are not auth-gated — the library is meant to be shared across the boat. Only the state-changing routes (upload, delete, purge) require a principal. If you add a route that mutates state, it gates itself the same way; if you add one that only reads, keep it consistent with the existing open read paths.
