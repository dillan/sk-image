...
SK Image needs **Node 22.13 or newer** because it uses Node's built-in SQLite. Run `node -v`; if it is below 22.13, upgrade Node on the server and restart Signal K. Some appliances lag behind on Node — notably the **Victron Cerbo GX / Venus OS**, which ships Node 20 — and can't run SK Image until they update to Node 22.13 or newer.
